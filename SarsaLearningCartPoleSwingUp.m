...
MAX_STEPS = 120000;      % Maximum number of steps (time) until Task success
...
while(steps<=MAX_STEPS && failures<+MAX_FAILURES)
    ...
    if(newBox == 163)
        ...
        % Reset Cart-pole system. Here, pole is reset to its Stable
        ...
        % Swing up action till 1 degree from Unstable equilibrium. At 1
        % degree from Unstable equilibrium, we switch to stabilization.
        while((theta<=(-pi/180) || theta>=(pi/180)))
            
            z = z+1;
            
            % Find "Force" required using the Energy Method of Swing-up
            F = SwingUpController2(theta,thetaDot,thetaacc,x,xDot);
            
            % Simulate Cart-pole (Environment) dynamics for swing up
        ...
        % Get state index for reset state after Swing-up
    ...
    % SARSA update for State-action value function
